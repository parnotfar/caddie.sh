...
3. Add `caddie_<module>_help` and register commands. Core generates command and namespace help from that metadata. Optionally add one `caddie_<module>_command_help <full-command>` hook to replace the generated output with richer module-specific help; the dispatcher routes both `<command> --help` and `<command>:help` to it.
