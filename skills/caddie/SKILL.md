...
3. **Inspect one command** — Every installed module with registered command metadata supports both `caddie <module>:<command> --help` and `caddie <module>:<command>:help`. Command namespaces use the same forms, such as `caddie js:project --help` and `caddie js:project:help`.
